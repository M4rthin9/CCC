...
export function addNote(db: D1Database, note: {
  ref: string;
  text: string;
  user: string;
  timestamp: string;
  ...
  return db.prepare(
    `INSERT INTO ${TABLES.notes} (ref, text, user, timestamp, createdAt)
     VALUES (?, ?, ?, ?, ?)`
  ).bind(note.ref, note.text, note.user, note.timestamp, note.createdAt).run().then(() => undefined);
}
...
export function getNotesByRef(db: D1Database, ref: string): Promise<Note[]> {
  ...
  ).bind(ref).all<Note>().then(res => (res.results ?? []).map(n => ({
    ...n,
    text: n.text ?? '',
    user: n.user ?? '',
