...
export function reservationRowToObject(row: unknown): Reservation {
  ...
  for (const col of RESERVATION_COLUMNS) {
    ...
    if (['visitorCount', 'totalPersons', 'total', 'adultCount', 'child5to8Count', 'childUnder5Count', 'version'].includes(col)) {
      out[col] = Number(v);
    } else {
      out[col] = String(v);
    }
...
export function getActiveReservations(db: D1Database): Promise<Reservation[]> {
  return db.prepare(
    `SELECT ${RESERVATION_COLUMNS.join(', ')} FROM ${TABLES.reservations} ORDER BY rowid DESC`
  ).all<Record<string, unknown>>().then(res => (res.results ?? []).map(reservationRowToObject));
}

export function getArchivedReservations(db: D1Database): Promise<Reservation[]> {
  return db.prepare(
    `SELECT ${RESERVATION_COLUMNS.join(', ')}, archivedAt FROM ${TABLES.archive} ORDER BY rowid DESC`
...
export function getReservationByRef(db: D1Database, ref: string): Promise<Reservation | null> {
  return db.prepare(
    `SELECT ${RESERVATION_COLUMNS.join(', ')} FROM ${TABLES.reservations} WHERE ref = ?`
  ).bind(ref).first<Record<string, unknown>>().then(r => (r ? reservationRowToObject(r) : null));
}

export function getReservationsByRefs(db: D1Database, ref: string): Promise<Reservation[]> {
  return db.prepare(
    `SELECT ${RESERVATION_COLUMNS.join(', ')} FROM ${TABLES.reservations} WHERE ref = ? ORDER BY rowid`
  ).bind(ref).all<Record<string, unknown>>().then(res => (res.results ?? []).map(reservationRowToObject));
}
...
export function getReservationsByPrisonerId(db: D1Database, prisonerId: string): Promise<Reservation[]> {
  ...
  ).bind(prisonerId).all<Record<string, unknown>>().then(res => (res.results ?? []).map(reservationRowToObject));
}
...
export function findDuplicateActiveBooking(
  ...
  sql += ' LIMIT 1';
  return db.prepare(sql).bind(...params).first<{ ref: string }>().then(r => (r ? r.ref : null));
}
...
export function countReservationsByDate(db: D1Database): Promise<Record<string, number>> {
  return db.prepare(
    ...
    .all<{ visitDateISO: string; c: number }>().then(res => {
...
export function updateReservationColumns(
  ...
  return db.prepare(
    `UPDATE ${TABLES.reservations} SET ${setSql} WHERE ref = ?`
  ).bind(...params, ref).run().then(() => undefined);
}

export function insertArchivedReservation(db: D1Database, data: Record<string, unknown>): Promise<void> {
  const cols = RESERVATION_COLUMNS.concat('archivedAt');
  const values = cols.map(c => data[c] !== undefined ? data[c] : '');
  return db.prepare(
...
export function deleteReservation(db: D1Database, ref: string): Promise<void> {
  return db.prepare(`DELETE FROM ${TABLES.reservations} WHERE ref = ?`).bind(ref).run().then(() => undefined);
}

export function clearAllReservations(db: D1Database): Promise<void> {
  return db.prepare(`DELETE FROM ${TABLES.reservations}`).run().then(() => undefined);
}
