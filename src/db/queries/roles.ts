...
export function getRoles(db: D1Database): Promise<RolePermission[]> {
  return db
    .prepare(`SELECT roleName, ${PERMISSION_COLUMNS.join(', ')} FROM ${TABLES.roles}`)
...
export function roleExists(db: D1Database, roleName: string): Promise<boolean> {
  return db
    .prepare(`SELECT 1 FROM ${TABLES.roles} WHERE lower(roleName) = lower(?)`)
    .bind(roleName)
    .first()
    .then((r) => !!r);
}
...
export function createRole(db: D1Database, roleName: string, permissions: string[]): Promise<void> {
  ...
  return db
    .prepare(`INSERT INTO ${TABLES.roles} (${cols.join(', ')}) VALUES (${cols.map(() => '?').join(', ')})`)
    .bind(...values)
    .run()
    .then(() => undefined);
}

export async function hasPermission(db: D1Database, username: string, perm: string): Promise<boolean> {
