...
export function getAllUsers(db: D1Database): Promise<PublicUser[]> {
  return db
    .prepare(`SELECT username, role, displayName, createdAt FROM ${TABLES.users} ORDER BY rowid`)
    .all<PublicUser>()
    .then((res) =>
      (res.results ?? []).map((u) => ({
        username: u.username,
...
export function createUser(
  ...
  return db
    ...
    .bind(u.username, u.password, u.role, u.displayName, u.createdAt)
    .run()
    .then(() => undefined);
}

export function updateUserColumns(db: D1Database, username: string, cols: Array<[string, unknown]>): Promise<void> {
  if (cols.length === 0) return Promise.resolve();
  const setSql = cols.map(([c]) => `${c} = ?`).join(', ');
  return db
    .prepare(`UPDATE ${TABLES.users} SET ${setSql} WHERE lower(username) = lower(?)`)
    .bind(...cols.map(([, v]) => v), username)
    .run()
    .then(() => undefined);
}
...
export function deleteUser(db: D1Database, username: string): Promise<void> {
  return db
    .prepare(`DELETE FROM ${TABLES.users} WHERE lower(username) = lower(?)`)
    .bind(username)
    .run()
    .then(() => undefined);
}
...
export function userExists(db: D1Database, username: string): Promise<boolean> {
  return db
    .prepare(`SELECT 1 FROM ${TABLES.users} WHERE lower(username) = lower(?)`)
    .bind(username)
    .first()
    .then((r) => !!r);
}
