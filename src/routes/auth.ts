...
export async function handleLogin(env: Env, body: Record<string, unknown>, meta: { ip: string; userAgent: string }): Promise<Record<string, unknown>> {
  const rawUsername = String(body.username || '').trim();
  const username = rawUsername.toLowerCase();

  if (!(await checkRateLimit(env.CACHE_KV, rateLimitKey('login', username), MAX_LOGIN_ATTEMPTS, LOGIN_RATE_LIMIT_TTL))) {
  ...
  if (String(user.password || '').indexOf('sha256$') !== 0) {
    await updateUserColumns(env.DB, rawUsername, [['password', await hashPassword(env, rawUsername, String(body.password || ''))]]);
    await cacheRemove(env.CACHE_KV, cacheKeyUser(env, rawUsername));
  }
  ...
  const [accessToken, refreshToken] = await Promise.all([
    signAccessToken(env.JWT_SECRET, { username: user.username, role: user.role, displayName: user.displayName || user.username }),
...
export async function handleChangePassword(env: Env, body: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  if (!newPassword || newPassword.length < 6) return { status: 'error', message: 'รหัสผ่านต้องมีความยาวอย่างน้อย 6 ตัวอักษร' };
...
export async function handleRefresh(env: Env, body: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  const accessToken = await signAccessToken(env.JWT_SECRET, { username: user.username, role: user.role, displayName: user.displayName || user.username });
  return { status: 'ok', accessToken, user: { username: user.username, role: user.role, displayName: user.displayName || user.username } };
}
