...
/** Blue header band, clipped to the card's rounded top. */
const BAND_H = 124;

/** Thai QR Payment logo — box matches the 750×210 art aspect so it fills the
 *  box exactly (no letterboxing) and centers in the band. */
const HEADER_ART_W = 232;
const HEADER_ART_H = 65;
const HEADER_ART_X = Math.round((CARD_W - HEADER_ART_W) / 2);
const HEADER_ART_Y = Math.round((BAND_H - HEADER_ART_H) / 2);

/** PromptPay sub-mark — box matches the 384×130 PNG aspect. */
const SUBMARK_W = 280;
const SUBMARK_H = 95;
const SUBMARK_X = Math.round((CARD_W - SUBMARK_W) / 2);
const SUBMARK_Y = 136;

/** Divider line separating the branding zone from the QR zone. */
const DIVIDER_Y = 246;
const DIVIDER_X = 60;
const DIVIDER_W = CARD_W - DIVIDER_X * 2;

/** QR on a clean white panel with a hairline frame. */
const QR_X = 60;
const QR_Y = 260;
const QR_SIZE = 480;

/** Caption row (merchant + amount), centered under the QR panel. */
const CAPTION_MERCHANT_Y = 782;
const CAPTION_AMOUNT_Y = 818;
...
export async function renderPromptPayCardSvg(payload: string, options?: PromptPayCardOptions): Promise<string> {
  ...
  const caption =
    options?.showCaption === false
      ? ''
      : `<text x="300" y="${CAPTION_MERCHANT_Y}" text-anchor="middle" font-family="Inter, system-ui, sans-serif" font-size="22" font-weight="600" fill="#33526E">${xmlEscape(options?.merchantName ?? '')}</text>` +
        `<text x="300" y="${CAPTION_AMOUNT_Y}" text-anchor="middle" font-family="Inter, system-ui, sans-serif" font-size="34" font-weight="700" fill="#00427A">${xmlEscape(options?.amountLabel ?? '')}</text>`;
  ...
  return (
    ...
    `<filter id="cardShadow" x="-20%" y="-20%" width="140%" height="160%"><feDropShadow dx="0" dy="6" stdDeviation="14" flood-color="#0B2545" flood-opacity="0.14"/></filter>` +
    `<filter id="panelShadow" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="3" stdDeviation="8" flood-color="#0B2545" flood-opacity="0.10"/></filter>` +
    `<clipPath id="cardClip"><rect width="${CARD_W}" height="${CARD_H}" rx="${CARD_RX}"/></clipPath>` +
    `</defs>` +
    `<rect width="${CARD_W}" height="${CARD_H}" rx="${CARD_RX}" fill="#ffffff" filter="url(#cardShadow)"/>` +
    `<g clip-path="url(#cardClip)">` +
    `<rect width="${CARD_W}" height="${BAND_H}" fill="#00427A"/>` +
    `</g>` +
    ...
    `<line x1="${DIVIDER_X}" y1="${DIVIDER_Y}" x2="${DIVIDER_X + DIVIDER_W}" y2="${DIVIDER_Y}" stroke="#E9EEF4" stroke-width="2"/>` +
    `<rect x="${QR_X}" y="${QR_Y}" width="${QR_SIZE}" height="${QR_SIZE}" rx="10" fill="#ffffff" stroke="#E3EAF2" stroke-width="1.5" filter="url(#panelShadow)"/>` +
